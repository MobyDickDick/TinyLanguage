...
- Nutze `examples/rosetta/copy_rosetta_samples.py`, um fehlende Rosetta-Code-Python-Dateien in ein Zielverzeichnis zu kopieren. Beispiel mit Filter und Dry-Run: `python examples/rosetta/copy_rosetta_samples.py ~/rosetta_import --include fib --include fizz --limit 5 --delay 0 --dry-run`. Das Skript vergleicht Dateinamen im Standardquellordner `examples/rosetta/python` mit dem Ziel, bietet Prefix-Filter für Teilmengen und wartet optional zwischen Kopiervorgängen.
- Mit `--transpile` stößt das Skript nach dem Kopieren automatisch die Übersetzung an. Destination und Quelle lassen sich anpassen, z.B.: `python examples/rosetta/copy_rosetta_samples.py examples/rosetta/python --transpile --transpile-dest examples/rosetta/expected`.
- Das Transpilieren lässt sich auch direkt ausführen: `python -m transpile_rosetta --source examples/rosetta/python --dest examples/rosetta/expected`. Passe `--dest` an, falls die TinyLanguage-Dateien in einem anderen Ordner landen sollen.
- Eine bereits erzeugte Sammlung der vorhandenen Python-Beispiele liegt unter `src_tiny/from_python/` – sie wurde per `python -m transpile_rosetta --dest $(pwd)/src_tiny/from_python` generiert und kann direkt mit `python src/tiny_language.py src_tiny/from_python/<name>.tiny` ausgeführt werden.
