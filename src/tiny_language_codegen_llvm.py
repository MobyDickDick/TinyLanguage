...
class LLVMCodeGenerator:
    ...
    def _compile_function(self, func: FunctionIR, signature: _ResolvedFunctionSignature) -> List[str]:
        ...
        lines: List[str] = []
        lines.append(f"def {signature.return_type} @{func.name}({param_sig}) {{")
        lines.append("entry:")
        lines.extend(self._prologue)
        lines.append(f"  br label %{label_map[0]}")
        lines.extend(self._body)
    ...
    def _runtime_helpers(self) -> List[str]:
        return [
            ...
            "  ret i64 %int",
            "}",
            "def i64 @new(i64 %size) {",
            "entry:",
            "  %ptr = call i64 @__new(i64 %size)",
            "  ret i64 %ptr",
            "}",
            "def i64 @__heap_bounds_error(i64 %idx, i64 %size) {",
            "entry:",
            ...
            "  ret i64 0",
            "}",
            "def i64 @heap_get(i64 %ptr, i64 %idx) {",
            "entry:",
            ...
            "  ret i64 %value",
            "}",
            "def i64 @heap_set(i64 %ptr, i64 %idx, i64 %value) {",
            "entry:",
            "  %data = inttoptr i64 %ptr to i64*",
            "  %base = getelementptr i64, i64* %data, i64 -1",
            "  %size = load i64, i64* %base",
            ...
            "  ret i64 0",
            "}",
            "def i8* @heap_get_str(i64 %ptr, i64 %idx) {",
            "entry:",
            "  %raw = call i64 @heap_get(i64 %ptr, i64 %idx)",
            "  %cast = inttoptr i64 %raw to i8*",
            "  ret i8* %cast",
            "}",
            "def double @heap_get_double(i64 %ptr, i64 %idx) {",
            "entry:",
            "  %raw = call i64 @heap_get(i64 %ptr, i64 %idx)",
            "  %cast = bitcast i64 %raw to double",
            "  ret double %cast",
            "}",
            "def i1 @heap_get_bool(i64 %ptr, i64 %idx) {",
            "entry:",
            "  %raw = call i64 @heap_get(i64 %ptr, i64 %idx)",
            "  %cast = trunc i64 %raw to i1",
            "  ret i1 %cast",
            "}",
            "def i64 @heap_set_str(i64 %ptr, i64 %idx, i8* %value) {",
            "entry:",
            "  %cast = ptrtoint i8* %value to i64",
            "  %_ignored = call i64 @heap_set(i64 %ptr, i64 %idx, i64 %cast)",
            "  ret i64 0",
            "}",
            "def i64 @heap_set_double(i64 %ptr, i64 %idx, double %value) {",
            "entry:",
            "  %cast = bitcast double %value to i64",
            "  %_ignored = call i64 @heap_set(i64 %ptr, i64 %idx, i64 %cast)",
            "  ret i64 0",
            "}",
            "def i64 @heap_set_bool(i64 %ptr, i64 %idx, i1 %value) {",
            "entry:",
            "  %cast = zext i1 %value to i64",
            "  %_ignored = call i64 @heap_set(i64 %ptr, i64 %idx, i64 %cast)",
            "  ret i64 0",
            "}",
            "def i64 @delete(i64 %ptr) {",
            "entry:",
            "  %data = inttoptr i64 %ptr to i64*",
            "  %base = getelementptr i64, i64* %data, i64 -1",
