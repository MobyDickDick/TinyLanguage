...
def test_smoke_stdlib_module_usage() -> None:
    ...
    source = "\n".join(
        [
            ...
            "def _rows_index = 0;",
            "while (_rows_index < Collections.len(rows)) {",
            "  def _row = heap_get(rows, _rows_index);",
            "  def _cleanup_row = delete(_row);",
            "  _rows_index = _rows_index + 1;",
            "}",
