...
- The TinyLanguage VS Code extension now forwards the `env` block from your `launch.json` to the debug adapter itself. You can enable adapter logging by adding `"env": { "TINYLANGUAGE_DAP_LOG": "/tmp/tiny_dap.log", "TINYLANGUAGE_DAP_STDERR": "1" }` to your TinyLanguage configuration—no extra global shell exports required.
