...
def main() -> int:
    ...
    if args.merge_variants:
        for group in group_image_variants(images):
            ...
            reference_gray = load_grayscale_image(representative)
            quality_rows.append(
                QualityRow(
                    image=representative.name,
                    svg=out.name,
                    width=len(reference_gray[0]) if reference_gray else 0,
                    height=len(reference_gray),
                    avg_error_per_pixel=compute_svg_grayscale_mae(reference_gray, out),
                )
            )
        ...
        for image in images:
            ...
            reference_gray = load_grayscale_image(image)
            quality_rows.append(
                QualityRow(
                    image=image.name,
                    svg=out.name,
    ...
    write_quality_list(quality_rows, args.quality_list)
    print(f"quality-list: {args.quality_list} ({len(quality_rows)} rows)")
    return 0
