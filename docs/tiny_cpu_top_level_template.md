...
| 9 | Status | `Datapath.ZERO`, `NEGATIVE` und Valid-Signale | `FetchDecode`/Fehlerlogik | 1 je Netz | Die Ladevalidität folgt Konstante oder Speicher, `NOT` übernimmt `Datapath.ACC_VALID_OUT`, und `INPUT` hat mit `INPUT_VALID` die letzte Auswahlpriorität. Als Nächstes die binären Familien einzeln ergänzen; Sprungbedingungen separat mit 0, negativem und positivem Wert prüfen. |
