...
def test_cancellation_token_stops_linked_spawn(run_tiny_source):
    out = run_tiny_source(
        textwrap.dedent(
            ...
            define token = Async.token();
            define ptr = new(1);
            heap_set(ptr, 0, 0);

            define handle = spawn worker(token, ptr);
            define linked = Async.link(token, handle);
            define cancelled = Async.cancel(token, "stop");

            define status = join(handle, 1000);
...
def test_cancellation_token_idempotent_and_immediate(run_tiny_source):
    out = run_tiny_source(
        textwrap.dedent(
            ...
            define token = Async.token();
            define handle = spawn quick(token);
            define linked = Async.link(token, handle);

            define first = Async.cancel(token, "first");
            define second = Async.cancel(token, "second");

            define status = join(handle, 1000);

            define late = spawn quick(token);
            define late_linked = Async.link(token, late);
            define late_status = join(late, 1000);
