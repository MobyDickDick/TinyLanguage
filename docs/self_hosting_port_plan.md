...
- Keep Python and Tiny files side by side (e.g., [src_tiny/lexer.tiny](src_tiny/lexer.tiny)) with matching module names and short docstrings describing parity with the Python original.
...
- Interpreter parity: for each migrated module, run the relevant sections of [tests/test_tiny_language.py](tests/test_tiny_language.py) through both Python and Tiny entry points (document any expected deviations).
...
- **AST + runtime parity:** ✅ Completed alignment for [src_tiny/tiny_language_ast.tiny](src_tiny/tiny_language_ast.tiny), [src_tiny/tiny_language_runtime.tiny](src_tiny/tiny_language_runtime.tiny), and [src_tiny/tiny_language_eval.tiny](src_tiny/tiny_language_eval.tiny) with their Python counterparts, keeping built-in behaviors and error messages delegated to the Python runtime for parity.
- **Native backend parity:** ✅ Mirrored [src/native_ir.py](src/native_ir.py), [src/tiny_language_codegen_native.py](src/tiny_language_codegen_native.py), and [src/native_vm.py](src/native_vm.py) into their `src_tiny/` equivalents, keeping opcode names, formatting, and error messages aligned.
- **Transpiler + CLI parity:** ✅ ensure [src_tiny/tiny_language_transpilers.tiny](src_tiny/tiny_language_transpilers.tiny), [src_tiny/tiny_language_cli.tiny](src_tiny/tiny_language_cli.tiny), and [src_tiny/tiny_lang_cli.tiny](src_tiny/tiny_lang_cli.tiny) match Python outputs/flags; add snapshot parity checks alongside existing tests (see [tests/detailtests/test_tiny_transpilers.py](tests/detailtests/test_tiny_transpilers.py) and [tests/detailtests/test_tiny_language_cli_self_host.py](tests/detailtests/test_tiny_language_cli_self_host.py)).
...
| --- | --- | --- | --- | --- |
| Lexer + parser | [src/tiny_language_lexer.py](src/tiny_language_lexer.py), [src/tiny_language_parser.py](src/tiny_language_parser.py) | [src_tiny/tiny_language_lexer.tiny](src_tiny/tiny_language_lexer.tiny), [src_tiny/tiny_language_parser.tiny](src_tiny/tiny_language_parser.tiny) | ✅ Complete | Lexer/parser parity is implemented; see the parity notes below for the validation checklist. Smoke-tested via [tests/test_tiny_parser_self_host.py](tests/test_tiny_parser_self_host.py) and [tests/test_language_spec_grammar.py](tests/test_language_spec_grammar.py). |
| AST + runtime | [src/tiny_language_ast.py](src/tiny_language_ast.py), [src/tiny_language_runtime.py](src/tiny_language_runtime.py), [src/tiny_language_eval.py](src/tiny_language_eval.py) | [src_tiny/tiny_language_ast.tiny](src_tiny/tiny_language_ast.tiny), [src_tiny/tiny_language_runtime.tiny](src_tiny/tiny_language_runtime.tiny), [src_tiny/tiny_language_eval.tiny](src_tiny/tiny_language_eval.tiny) | ✅ Complete | Runtime/eval continue delegating to the stitched Python module for behavior and error message parity. Interpreter coverage includes [tests/test_tiny_language.py](tests/test_tiny_language.py). |
| Linter | [src/tiny_language_linter.py](src/tiny_language_linter.py) | [src_tiny/tiny_language_linter.tiny](src_tiny/tiny_language_linter.tiny) | ✅ Complete | Python/Tiny parity verified in [tests/test_linter_parity.py](tests/test_linter_parity.py). |
| Transpilers | [src/tiny_language_transpilers.py](src/tiny_language_transpilers.py) | [src_tiny/tiny_language_transpilers.tiny](src_tiny/tiny_language_transpilers.tiny) | ✅ Complete | Renderers/parsers mirrored for Python/Julia/JS/C++; parity snapshot tests cover renderer output. |
| Native backend | [src/native_ir.py](src/native_ir.py), [src/tiny_language_codegen_native.py](src/tiny_language_codegen_native.py), [src/native_vm.py](src/native_vm.py) | [src_tiny/native_ir.tiny](src_tiny/native_ir.tiny), [src_tiny/tiny_language_codegen_native.tiny](src_tiny/tiny_language_codegen_native.tiny), [src_tiny/native_vm.tiny](src_tiny/native_vm.tiny) | ✅ Complete | Opcode names, formatting, and error messages match the Python backend outputs. |
| CLI / LSP | [src/tiny_language_cli.py](src/tiny_language_cli.py), [src/tiny_lang_cli.py](src/tiny_lang_cli.py), [src/language_server.py](src/language_server.py), [src/language_server_cli.py](src/language_server_cli.py) | [src_tiny/tiny_language_cli.tiny](src_tiny/tiny_language_cli.tiny), [src_tiny/tiny_lang_cli.tiny](src_tiny/tiny_lang_cli.tiny), [src_tiny/language_server.tiny](src_tiny/language_server.tiny), [src_tiny/language_server_cli.tiny](src_tiny/language_server_cli.tiny) | ✅ Complete | Tiny CLI exercises completions/hover/diagnostics through the Python language server helpers; Tiny language server helpers remain available for future wiring. |

### Lexer + parser parity notes

- The Tiny scaffolding now lives in [src_tiny/tiny_language_lexer.tiny](src_tiny/tiny_language_lexer.tiny) and [src_tiny/tiny_language_parser.tiny](src_tiny/tiny_language_parser.tiny). Both files mirror the public entry points (token container, keyword/builtin sets, parser shell) so future ports can plug in logic without reshuffling signatures.
