...
| 4 | Akkumulator-Steuerung | passender `FetchDecodeControls`-Ausgang | passender `Datapath`-Eingang | 1 je Netz | `LOAD_CONST` und `LOAD_ADDRESS` erreichen `ACC_LOAD` über das benannte ODER-Gatter `ACC_LOAD_REQUEST`; weitere Ladeursachen werden gruppenweise ergänzt und dürfen nicht durch direktes Zusammenschalten mehrerer Ausgänge entstehen. |
...
| `ACC_LOAD_REQUEST` | `FetchDecodeControls.LOAD_CONST (650,370)` und `LOAD_ADDRESS (650,390)` über getrennte ODER-Eingänge | `Datapath.ACC_LOAD (720,180)` |
