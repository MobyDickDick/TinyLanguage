...
def _read_lockfile(lock_path: Path) -> dict[str, Any]:
    if not lock_path.is_file():
        raise SystemExit(f"Missing lockfile: {lock_path}")
    return tomllib.loads(lock_path.read_text(encoding="utf-8"))


def _lockfile_entries(lock_data: dict[str, Any]) -> list[dict[str, Any]]:
...
def _write_vendor_readme(
    vendor_root: Path,
    lock_data: dict[str, Any],
    entries: list[dict[str, Any]],
) -> None:
    readme_path = vendor_root / "README.md"
    manifest_hash = lock_data.get("manifest_hash", "unknown")
    registry = lock_data.get("registry")
    lines = [
        "# Vendored TinyLanguage dependencies",
        "",
        "This directory was generated by `tiny pkg vendor`.",
        "",
        f"- Manifest hash: `{manifest_hash}`",
    ]
    if registry:
        lines.append(f"- Default registry: `{registry}`")
    lines.extend(
        [
            "",
            "## Packages",
    ...
    if entries:
        for entry in sorted(entries, key=lambda item: (item.get("source", ""), item.get("name", ""))):
            ...
            origin = entry.get("path") or entry.get("registry") or entry.get("url") or "unknown"
            lines.append(f"- `{name}` `{version}` ({source}) — {origin}")
    else:
        lines.append("- _No dependencies listed in tiny.lock._")
...
def _cmd_vendor(args: argparse.Namespace) -> int:
    ...
    _write_vendor_readme(vendor_root, lock_data, entries)
