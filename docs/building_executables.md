...
1. **Install PyInstaller** (inside your virtual environment):
   ...
   ```
2. **Run PyInstaller from the repo root** so the bundled data files match TinyLanguage’s dynamic loader:
   ```bash
   pyinstaller --onefile ^
     --name tiny_language ^
...
3. **Pick up the executable** from `dist/tiny_language.exe`.
