...
| 9 | Status | `Datapath.ZERO`, `NEGATIVE` und Valid-Signale | `FetchDecode`/Fehlerlogik | 1 je Netz | Die Ladevalidität folgt Konstante oder Speicher, `NOT` übernimmt `Datapath.ACC_VALID_OUT`, `ADD_*` verlangt zusätzlich einen gültigen ausgewählten Operanden, und `INPUT` hat mit `INPUT_VALID` die letzte Auswahlpriorität. Als Nächstes `SUB_*` ergänzen; Sprungbedingungen separat mit 0, negativem und positivem Wert prüfen. |
